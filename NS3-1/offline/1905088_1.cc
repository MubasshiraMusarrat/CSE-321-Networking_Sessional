...
NS_LOG_COMPONENT_DEFINE("WifiStaticExample");

uint32_t nNodes = 20;
uint32_t nFlows = 10;
uint32_t nPacketsPerSec = 100;
uint32_t coverageArea = 1;
...
int main(int argc, char* argv[]){
    ...
    Ptr<OutputStreamWrapper>stream1  = asciiTraceHelper.CreateFileStream("scratch/static1.dat", std::ios::app);
    Ptr<OutputStreamWrapper>stream2  = asciiTraceHelper.CreateFileStream("scratch/static2.dat", std::ios::app);

    dataRate = 5* 1024 * 8;
    double p2pDataRate = dataRate*0.8;

    uint32_t Tx_Range = 5;
    uint32_t WifiRange = coverageArea * Tx_Range;
    ...
    phySender.Set("TxPowerStart",DoubleValue(WifiRange));
    phySender.Set("TxPowerEnd",DoubleValue(WifiRange));
    phySender.Set("TxGain",DoubleValue(15));
    phySender.Set("RxGain",DoubleValue(5));
    ...
    phyReceiver.Set("TxPowerStart",DoubleValue(WifiRange));
    phyReceiver.Set("TxPowerEnd",DoubleValue(WifiRange));
    phyReceiver.Set("TxGain",DoubleValue(15));
    phyReceiver.Set("RxGain",DoubleValue(5));
